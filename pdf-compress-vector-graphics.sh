...
for page_number in $(seq 60 100); do
  ...
  #page_pdf_path=$page_base_path.input.pdf
  ...
  if ! echo "$page_pdf_width" | grep -q "pt$"; then
    echo "error: expected page width and height in pt units, got width '$page_pdf_width' and height '$page_pdf_height' from 'pdfinfo $page_pdf_path'"
    exit 1
  fi
  page_pdf_width_pt=$(echo $page_pdf_width | sed 's/pt$//')
  page_pdf_height_pt=$(echo $page_pdf_height | sed 's/pt$//')
  ...
  for resolution in 600; do # i want 600dpi for printing (1200dpi would be too much)
    ...
    page_objects_scale_factor=$((resolution / page_objects_resolution))

    if (( page_objects_scale_factor > 1 )); then
      echo "page objects scale factor: $page_objects_scale_factor"
      # no? wrong output size?
      # page_png_path=$page_vectors_pdf_path.${resolution}dpi.png
      if false; then
        # TODO assert integer result
        page_objects_resolution=$((resolution / page_objects_scale_factor))
        page_objects_page_png_path=$page_png_path.${page_objects_resolution}dpi.png
        convert $page_png_path -scale $((100 / page_objects_scale_factor))% $page_objects_page_png_path
      fi
    ...
    if (( page_objects_scale_factor > 1 )); then
      for object_idx in ${!object_bbox_list[@]}; do
        ...
        object_bbox_x=$((object_bbox_x * page_objects_scale_factor))
        object_bbox_y=$((object_bbox_y * page_objects_scale_factor))
        object_bbox_l=$((object_bbox_l * page_objects_scale_factor))
        object_bbox_t=$((object_bbox_t * page_objects_scale_factor))
    ...
    for object_idx in ${!object_bbox_list[@]}; do
      ...
      object_bbox=${object_bbox_list[object_idx]}
      convert $page_png_path -crop $object_bbox +repage $object_png_path
      ...
      #for jp2_quality in $(seq $jp2_quality_min $jp2_quality_inc $jp2_quality_max); do
        ...
        convert $object_png_path -quality $jp2_quality $jp2_path
...
combined_output_pdf_path=combined.output.pdf
